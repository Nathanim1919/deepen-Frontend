...
export class AIController {
  ...
  static async chat(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      // Get user's Gemini API key (still needed for embeddings/retrieval)
      const apiKey = await UserService.getGeminiApiKey(user.id);

      if (!apiKey) {
        res.write(
          `data: ${JSON.stringify({ error: "API key is required for document retrieval", code: "API_KEY_REQUIRED" })}\n\n`,
        );
        res.end();
        return;
      }
      ...
      // Get the OpenRouter stream from our service function
      const stream = await processConversationStream(
        user,
        apiKey,
      ...
      // Write each chunk from the OpenRouter stream to the response
